...
def generate_maze(n):
    ...
    global maze, start, end, visited, choices
    # initialize maze
    ...
    start = (random.randint(1, n-1), random.randint(1, n-1))
    choices = []
    dfs(start, 0)
    e = random.randint(1, len(choices))
    print(len(choices))
    end = choices[e]
    if watch: print_maze(maze)

def dfs(curr, maze_dist):
    global maze, visited, watch, choices
    maze_dist = maze_dist + 1
    ...
    for d in directions: # try every direction
        ...
        if 1 <= neighbor[0] < n and 1 <= neighbor[1] < n and not visited[neighbor[0]][neighbor[1]]:
            if watch:
                print_maze(maze)
                time.sleep(0.1)
            connectCells(curr, neighbor)
            dfs(neighbor, maze_dist)
    return
...
def maze_to_arr(maze):
    m = []
    for row in maze:
        m_r=[]
        for col in row:
            if(col==0): m_r.append('  ')
            elif(col==1): m_r.append(' |')
            elif(col==2): m_r.append('_ ')
            elif(col==3): m_r.append('_|')
        m.append(m_r)
    return m
...
def main():
    ...
    while not q == 'q':
        ...
        q = input("q to Quit maze generating with DFS: ")
